...
public class StateMachineConfig extends EnumStateMachineConfigurerAdapter<States,Events> {
    ...
    public void configure(StateMachineTransitionConfigurer<States, Events> transitions) throws Exception {
        transitions
                .withExternal()
                .source(States.UNPAID) //来源状态
                .target(States.WAITING_FOR_RECEIVE) //目标状态
    ...
    public void configure(StateMachineConfigurationConfigurer<States, Events> config) throws Exception {
        config
                .withConfiguration()
                .listener(listener()); //调用了下一个内容创建的监听器实例，用来处理各个各个发生的状态迁移事件
    }

    /**
     * 用来创建StateMachineListener状态监听器的实例
     * 在该实例中会定义具体的状态迁移处理逻辑，上面的实现中只是做了一些输出，
     * 实际业务场景会会有更负责的逻辑，所以通常情况下，
